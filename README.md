...
* **Contents**:
    * `:feature:*`: Concrete business feature modules, containing **Composable screens**,
      `ViewModels` (MVI), and feature-specific navigation graphs.
    * `:shared:designsystem`: Defines the basic visual specifications of the app (colors, themes,
      fonts) as Composable `Theme` and functions.
    * `:shared:ui`: Contains reusable composite **Composables** and UI-related utilities.
...
3. **Create UI**: In `:feature:settings`, create a `SettingsScreen.kt` file containing your main
   Composable. Create a `SettingsViewModel` that will be injected into your screen using
   `hiltViewModel()`. The screen will observe state from the ViewModel (
   `collectAsStateWithLifecycle`) and render the UI accordingly.
4. **Integrate Navigation**: In the `:app` module's main `NavHost`, add a new destination by calling
   `composable(route = SettingRoute) { SettingsScreen() }`. Navigate to this new screen by calling
   `navController.navigate("settings")` from another screen.
